...
class Events {
  ...
  static async editHandler() {
    try {
      this.formTitle.innerHTML = this.text.updateUser + this.currentUser.name;
  ...
  static async changeEnv() {
    try {
      if (this.envSelect.value != "") {
        const response = await ApiOperations.updateApi(this.envSelect.value);
        if (response === "Success") {
          ...
          this.clearForm();
          this.updateTablebtn("add");
          this.createModal(response, "Enviroment changed successfully");
        } else {
          this.createModal("Abort", response);
        }
      }
